...
Thetagamma = 0.2; % covariance of expected value of 
Theta = 0.5; % covariance of Weiner process
mu = 1; % lagrange multiplier
...
Aaug = mcv_ol_matrix(Abar,Bbar,Cbar,Q1bar,R,mu,Thetagamma,Theta);
Abarc{kk}=Abar;
Bbarc{kk}=Bbar;
Cbarc{kk}=Cbar;
Fbarc{kk}=Fbar;
...
for kk = 1:mdimS2
gamma1opt(kk) = -(1/2*mu)*(Bbarc{kk}.')*rhobar(:,kk);
end 
gamma1opt=gamma1opt.';

gamma1leader = [T,gamma1opt];
...
Jleader(kk) = tf*(xtraj(kk,:)*Q1*(xtraj(kk,:)).'+ ...
    gamma1opt(kk,:)*(gamma1opt(kk,:).')+gamma2opt(kk,:)*R*(gamma2opt(kk,:).'))/length(T);
